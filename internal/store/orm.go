...
func NewOrm(database config.DatabaseConfig, debug bool) *Orm {
	...
	switch dialect {
	case "mysql":
		dsn = username + ":" + password + "@tcp(" + host + ":" + strconv.Itoa(port) + ")/" + dbName + "?charset=utf8mb4&parseTime=True&loc=Local"
		...
		orm.DB = db
	case "postgresql":
		dsn = "host=" + host + " user=" + username + " password=" + password + " dbname=" + dbName + " port=" + strconv.Itoa(port) + " sslmode=disable TimeZone=Asia/Shanghai"
...
func (orm *Orm) AddModels(values ...interface{}) error {
	...
	if len(values) > 0 {
		for _, val := range values {
			...
			switch rVal.Kind() {
			case reflect.Struct:
				models[utils.GetTypeName(rVal.Type())] = reflect.New(rVal.Type())
